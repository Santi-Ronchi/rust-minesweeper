...
/// una linea totalmente en blanco o con una cantidad incorrecta de elementos, arroja error de formato.
pub fn run(config_filename: String) -> Result<(), Box<dyn Error>> {
    ...
    for line_result in buffered_reader.lines() {
        ...
        if value.is_empty() || (line_length != value.len() && line_length != 0) {
            return Err("Error in line format!")?;
        }
        line_length = value.chars().count();
